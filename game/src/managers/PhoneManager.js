...
export class PhoneManager {
    ...
    /**
     * NPC Autonomous Tick — called every advanceWeek()
     *
     * - Favor decays if no contact for 4+ weeks
     * - Pending offers expire after 3 weeks
     * - NPC-to-NPC gossip
     */
    static npcAutonomousTick() {
        const state = GameState.state;

        PhoneManager.contacts.forEach(npc => {
            // Init memory if needed
            if (!npc.memory) {
                npc.memory = { witnessed: [], grudges: [], favors: [], lastContact: npc.lastContact || 0 };
            }
            ...
            if (Math.random() < 0.15 && npc.memory.witnessed.length > 0) {
                ...
                if (otherNPCs.length > 0) {
                    ...
                    if (!gossipTarget.memory) {
                        gossipTarget.memory = { witnessed: [], grudges: [], favors: [], lastContact: 0 };
                    }
