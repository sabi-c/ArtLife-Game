...
export class GameState {
    ...
    // ════════════════════════════════════════
    // Save / Load — Delegated to SaveManager.js
    // ════════════════════════════════════════
    static MAX_SLOTS = 5;

    static get activeSlot() { return SaveManager.activeSlot; }
    static set activeSlot(v) { SaveManager.activeSlot = v; }

    static save(slotIndex) {
        return SaveManager.save(GameState.state, () => GameState.getPortfolioValue(), slotIndex);
    }
    ...
    static getSaveSlots() { return SaveManager.getSaveSlots(); }
    static getMostRecentSlot() { return SaveManager.getMostRecentSlot(); }
    static deleteSave(slotIndex) { return SaveManager.deleteSave(slotIndex); }
    static hasSave() { return SaveManager.hasSave(); }

    static autoSave() {
        if (GameState.state) {
            SaveManager.save(GameState.state, () => GameState.getPortfolioValue(), SaveManager.activeSlot);
        }
    }
    static seedDemoSave() { return SaveManager.seedDemoSave(); }
